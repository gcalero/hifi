//
//  Created by Cristian Duarte & Gabriel Calero on 2018/09/26
...
public:
    ...
    virtual DisplayPluginList getDisplayPlugins() override {
        ...
        std::call_once(once, [&] {
            DisplayPluginPointer plugin(new DaydreamDisplayPlugin());
            if (plugin->isSupported()) {
                _displayPlugins.push_back(plugin);
            }
            plugin = DisplayPluginPointer(new Android2DDisplayPlugin());
            if (plugin->isSupported()) {
                _displayPlugins.push_back(plugin);
            }
        });
